...
def get_sections_with_tunes(book: Book, tunes):
    ...
    for tune in tunes:
        ...
        if not section_id in sections:
            print(
                f"WARNING: Section {section_id} required by {tune.get('title')} does not exist in the book"
            )
            continue
...
def render_all(
    book: Book, grouped_tunes_sections, template_name, nomusic=False, location=location
):
    environment = Environment(loader=FileSystemLoader("jinja"))
    environment.filters["newline"] = lambda s: s.replace("\n", "\\\\ ") if s else ""
...
def get_tunes():
    def filter(tune):
        ...
        return (len(sys.argv) == 2 or tune.get("id") in sys.argv[2:]) and tune.get(
            "hidden", "false"
        ) != "true"
...
def main():
    ...
    tunes_with_scores = [t for t in tunes if t.get("section") != "canu"]
    save(render_each(tunes_with_scores, "tune.ly"), output_location + "{id}.ly")
    save(
        render_each(tunes_with_scores, "tune-in-dots.lytex"),
        output_location + "dots/{id}.lytex",
    )
    save(
        render_each(tunes_with_scores, "tune-in-guitar.lytex"),
        output_location + "guitar/{id}.lytex",
    )
    save(
        render_each(tunes_with_scores, "tune-in-mandolin.lytex"),
        output_location + "mandolin/{id}.lytex",
    )
    save(
        render_each(tunes_with_scores, "tune-in-gdad.lytex"),
        output_location + "gdad/{id}.lytex",
    )
    ...
    if len(sys.argv) == 2:
        ...
        save(
            render_all(
                book, sections, "book-in-dots.lytex", nomusic=True, location=location
            ),
            output_location + "book.nomusic.lytex",
        )
        save(
            render_all(book, sections, "book-in-dots.lytex", location=location),
            output_location + "book.dots.lytex",
        )
        save(
            render_all(book, sections, "book-in-guitar.lytex", location=location),
            output_location + "book.guitar.lytex",
        )
        save(
            render_all(book, sections, "book-in-mandolin.lytex", location=location),
            output_location + "book.mandolin.lytex",
        )
        save(
            render_all(book, sections, "book-in-gdad.lytex", location=location),
            output_location + "book.gdad.lytex",
        )
